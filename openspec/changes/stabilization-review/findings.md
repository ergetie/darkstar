...
- OQ5 — **Two estimators:** what is the relationship between the open-meteo library forecast (`backend/core/forecasts.py`) and the physics+ML hybrid? Is the library a sanity reference the hybrid should be clamped against? (Also Phase 3 / task 4.1.)

**Batching note:** OQ1–OQ4 likely become one `fix-pv-forecast-safety` change (clamp + cold-start + toggle, with physics calibration optional); OQ5 may fold into the Phase 3 architecture outcome.
