...
- **`small-correctness-cleanup` ↔ `executor-refactor` / Kepler work.** #19/#20 edit `kepler.py` (as does the promoted #34 inverter-AC change); #13/#21/#24 edit `engine.py`/`controller.py`. If `executor-refactor` (#36) or the M3/M4 resume runs concurrently, land the one-liners first or coordinate, to avoid re-touching the same lines.
