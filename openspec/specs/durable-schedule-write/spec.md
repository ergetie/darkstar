...
Schedule JSON is shared state between the planner, API, and executor. Writers must never expose a partially serialized document to readers.
...
#### Scenario: Concurrent reader sees a complete document
- **WHEN** a schedule writer updates `schedule.json` while an executor reads it
- **THEN** the executor SHALL observe either the previous complete document or the new complete document
- **AND** it SHALL never observe a partially serialized JSON document
...
#### Scenario: Serialization failure preserves the previous schedule
- **WHEN** schedule serialization raises an exception
- **THEN** the existing schedule file SHALL remain byte-identical
...
- **AND** the replace SHALL be performed with `os.replace` semantics onto the target
