...
Ensures every load `type` value the frontend can produce for EV chargers and water heaters is recognized as its own distinct type by the runtime load-disaggregation subsystem, and that config validation shares the same source of truth so accepted and understood values cannot silently drift apart.
