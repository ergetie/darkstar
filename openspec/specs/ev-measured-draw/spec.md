# ev-measured-draw Specification

## Purpose
Derive the measured per-phase current draw of each current-controlled EV charger every executor tick and use it as the effective baseline for amps adjustments, so surplus tracking and load balancing act on actual draw rather than the setpoint.
