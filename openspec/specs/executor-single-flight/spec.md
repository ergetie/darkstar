...
The scheduled executor loop and manual trigger endpoint can enter the same asynchronous tick from different threads. The tick must be single-flight.
...
- **WHEN** one tick is in flight in the loop thread and another entrant calls from a different thread
