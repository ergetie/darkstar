...
Make the runtime invariant monitors (shipped in `stabilization-review-2`) visible to the operator through a read-only UI panel, and remove the obsolete `data_quality_daily` table now that the runtime monitors are its live-era replacement.
