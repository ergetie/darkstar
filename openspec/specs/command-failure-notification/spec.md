...
Surface repeated executor action-command failures to the operator's notification channel, and ensure each `execution_log` row carries a top-level reason for why a tick failed, so hardware-command failures no longer go unnoticed.
