...
def diagnose_pv():
    ...
    if not weather_df.empty:
        ...
        print(f"   Weather Forecast: Rad={obs['shortwave_radiation_w_m2'].iloc[0]} W/m2, Cloud={obs['cloud_cover_pct'].iloc[0]}%")
    else:
    ...
    for feat in trained_features:
        if feat not in obs.columns: obs[feat] = 0.0
    ...
    try:
        ...
        impacts = sorted(zip(trained_features, contributions), key=lambda x: abs(x[1]), reverse=True)
