...
def _load_latest_policy_run(engine: LearningEngine) -> PolicyRunInfo | None:
    ...
    if row is None:
        return None
    return PolicyRunInfo(run_id=row[0], models_dir=row[1])
...
def _ascii_bar(delta: float, scale: float = 20.0) -> str:
    """Simple bar where 0 is center, positive = worse than MPC."""
    if scale <= 0:
        scale = 20.0
    width = 20
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare Antares policy, MPC, and Oracle costs over historical days.",
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    if run is None:
        print("[policy-cost] No antares_policy_runs entries found; train policy first.")
        return 1

    print("[policy-cost] Evaluating Antares policy cost vs MPC/Oracle:")
    print(f"  run_id:      {run.run_id}")
    print(f"  models_dir:  {run.models_dir}")

    days = _load_eval_days(engine, max_days=args.days)
    if not days:
        print("[policy-cost] No evaluation days; check data_quality_daily.")
        return 1
    ...
    for d in days:
        print(f"[policy-cost] Day {d}: running MPC, policy, Oracle...")
    ...
    if df.empty:
        print("[policy-cost] No cost records; aborting.")
        return 1

    # Drop days where MPC or policy cost is not finite.
    df = df.replace([np.inf, -np.inf], np.nan)
    valid_mask = df[["mpc_cost", "policy_cost"]].notna().all(axis=1)
    if not valid_mask.any():
        print("[policy-cost] All MPC/policy costs are NaN/inf; nothing to report.")
        return 1
    df_valid = df[valid_mask].copy()

    print("\n[policy-cost] Per-day cost comparison (SEK):")
    for _, row in df_valid.iterrows():
        ...
        delta_pm = pol - mpc
        bar = _ascii_bar(delta_pm, scale=max(1.0, abs(delta_pm)))
        line = f"  {d}: MPC={mpc:8.2f}  Policy={pol:8.2f}  ΔP-M={delta_pm:7.2f}  {bar}"
        if oracle is not None:
            line += f"  Oracle={float(oracle):8.2f}"
        print(line)

    print("\n[policy-cost] Aggregate stats:")
    mpc_total = float(df_valid["mpc_cost"].sum())
    pol_total = float(df_valid["policy_cost"].sum())
