...
def plot_timeseries(mpc_csv, kepler_csv, output_path):
    ...
    ax.plot(
        kepler_df[time_col],
        kepler_df["projected_soc_percent"],
        label="Kepler SoC %",
        color="#4CAF50",
        linewidth=2,
    )
    ...
    # MPC
    ax.fill_between(
        mpc_df[time_col], mpc_df["battery_charge_kw"], 0, color="red", alpha=0.3, label="MPC Charge"
    )
    ax.fill_between(
        mpc_df[time_col],
        -mpc_df["battery_discharge_kw"],
        0,
        color="green",
        alpha=0.3,
        label="MPC Discharge",
    ...
    ax.plot(
        kepler_df[time_col], kepler_net_power, color="blue", linewidth=1, label="Kepler Net Power"
    )
    ...
    ax.plot(
        kepler_df[time_col], kepler_df["grid_import_kw"], label="Kepler Import kW", color="orange"
    )
    ax.plot(
        kepler_df[time_col], -kepler_df["grid_export_kw"], label="Kepler Export kW", color="purple"
    )
    ...
    if "water_from_grid_kwh" in mpc_df.columns:
        mpc_water = (
            mpc_df["water_from_grid_kwh"]
            + mpc_df.get("water_from_pv_kwh", 0)
            + mpc_df.get("water_from_battery_kwh", 0)
        ) * 4  # kWh -> kW
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mpc", type=str, default="benchmark_mpc_timeseries.csv", help="MPC CSV file"
    )
    parser.add_argument(
        "--kepler", type=str, default="benchmark_kepler_timeseries.csv", help="Kepler CSV file"
    )
    parser.add_argument(
        "--output", type=str, default="benchmark_timeseries_plot.png", help="Output PNG file"
    )
