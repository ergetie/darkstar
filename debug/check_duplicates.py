    ...
    try:
        ...
        with sqlite3.connect(engine.db_path) as conn:
            ...
            total_rows = conn.execute(
                "SELECT COUNT(*) FROM slot_observations WHERE DATE(slot_start) = '2025-11-24'"
            ).fetchone()[0]
            unique_slots = conn.execute(
                "SELECT COUNT(DISTINCT slot_start) FROM slot_observations WHERE DATE(slot_start) = '2025-11-24'"
            ).fetchone()[0]
