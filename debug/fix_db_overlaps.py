...
def fix_overlaps():
    ...
    for i in range(len(df) - 1):
        ...
        if curr['slot_end'] > next_row['slot_start']:
            # Overlap detected
            print(f"Overlap: Row {curr['rowid']} ({curr['slot_start']} - {curr['slot_end']}) overlaps with Row {next_row['rowid']} ({next_row['slot_start']})")
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if modifications:
            ...
            for rowid, new_end in modifications:
                ...
                cursor.execute("UPDATE slot_observations SET slot_end = ? WHERE rowid = ?", (new_end_iso, rowid))
        conn.commit()
    print("Done.")
