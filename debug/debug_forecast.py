...
def debug_forecast(target_time_str):
    ...
    features = {
        ...
        "cloud_cover_pct": weather_row["cloud_cover_pct"].values[0] if "cloud_cover_pct" in weather_row else 0,
        "shortwave_radiation_w_m2": weather_row["shortwave_radiation_w_m2"].values[0] if "shortwave_radiation_w_m2" in weather_row else 0,
    ...
    importance = pd.DataFrame({
        'Feature': load_model.feature_name(),
        'Importance': load_model.feature_importance()
    }).sort_values(by='Importance', ascending=False)
    print(importance.head(5))

    print("\nPV Model:")
    importance_pv = pd.DataFrame({
        'Feature': pv_model.feature_name(),
        'Importance': pv_model.feature_importance()
    }).sort_values(by='Importance', ascending=False)
