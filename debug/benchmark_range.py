...
def run_day(date_str, config_path="config.yaml"):
    ...
    try:
        ...
        df_dis = (
            mpc_df["battery_discharge_kw"].sum() * 0.25 if "battery_discharge_kw" in mpc_df else 0
        )
        ...
        print(
            f" [Bal: L{df_load:.1f} P{df_pv:.1f} D{df_dis:.1f} C{df_char:.1f} I{df_imp:.1f}]",
            end="",
        )
        ...
        for _, row in mpc_df.iterrows():
            ...
            water = (
                row["water_from_grid_kwh"]
                + row["water_from_pv_kwh"]
                + row["water_from_battery_kwh"]
            )
        ...
        battery_cap = float(
            planner.config.get("system", {})
            .get("battery", {})
            .get("capacity_kwh", planner.config.get("battery", {}).get("capacity_kwh", 10.0))
        )

        kepler_df = kepler_result_to_dataframe(
            k_result, capacity_kwh=battery_cap, initial_soc_kwh=initial_soc
        )
            ...
            with sqlite3.connect(loader.db_path) as conn:
                ...
                if not mpc_df.empty:
                    ...
                    if not actuals_df.empty:
                        actuals_df["slot_start"] = pd.to_datetime(
                            actuals_df["slot_start"], utc=True
                        ).dt.tz_convert(loader.timezone)
        ...
        for t, row in kepler_df.iterrows():
            ...
            k_financial_cost += (imp * row["import_price_sek_kwh"]) - (
                exp * row["export_price_sek_kwh"]
            )
            k_import += imp
            k_export += exp
        ...
        for _, row in mpc_df.iterrows():
            imp = max(
                0.0,
                row["adjusted_load_kwh"]
                + row["battery_charge_kw"] * 0.25
                - row["adjusted_pv_kwh"]
                - row["battery_discharge_kw"] * 0.25,
            )  # Approx
        ...
        for t, row in mpc_df.iterrows():
            ...
            imp = max(0.0, net)
            exp = max(0.0, -net)
            mpc_financial_cost += (imp * row["import_price_sek_kwh"]) - (
                exp * row["export_price_sek_kwh"]
            )
...
def main():
    ...
    while current_date <= end_date:
        ...
        if res:
            ...
            print(
                f" Done. MPC: {res['mpc_cost']:.2f} SEK, Kepler: {res['kepler_cost']:.2f} SEK. Savings: {res['savings']:.2f} SEK"
            )
        else:
