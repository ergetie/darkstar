...
def plot_export_days(mpc_csv, kepler_csv, output_path):
    ...
    for i, day in enumerate(top_days):
        ...
        if not day_mpc.empty and 'import_price_sek_kwh' in day_mpc.columns:
             p1, = ax2.plot(day_mpc[time_col], day_mpc['import_price_sek_kwh'], color='black', linestyle=':', alpha=0.5, label='Price')
        ...
        p2, = ax3.plot(day_kepler[time_col], day_kepler['projected_soc_percent'], color='red', linewidth=2, label='SoC %')
        ax3.set_ylabel('SoC (%)')
        ...
        ax.fill_between(day_kepler[time_col], day_kepler['grid_export_kw'], 0, step='post', color='purple', alpha=0.6, label='Export')
        ax.fill_between(day_kepler[time_col], day_kepler['grid_import_kw'], 0, step='post', color='orange', alpha=0.4, label='Import')
