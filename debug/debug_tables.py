...
def inspect_tables():
    ...
    if not os.path.exists(db_path):
        if os.path.exists("test.db"): db_path = "test.db"
        else:
            print("No DB found.")
            return
    ...
    try:
        ...
        if count_obs > 0:
            cursor.execute("SELECT slot_start, pv_kwh, load_kwh FROM slot_observations ORDER BY slot_start DESC LIMIT 3")
        ...
        if count_fc > 0:
            cursor.execute("SELECT slot_start, pv_forecast_kwh, date(slot_start) FROM slot_forecasts ORDER BY slot_start DESC LIMIT 3")
