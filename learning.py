...
class LearningEngine:
    ...
    def _init_schema(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_slot_observations_start ON slot_observations(slot_start)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_slot_forecasts_start ON slot_forecasts(slot_start)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_learning_metrics_date ON learning_metrics(date)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_learning_runs_started ON learning_runs(started_at)")
    ...
    def etl_cumulative_to_slots(
        ...
        quality_flags: List[defaultdict[str, List[str]]] = [defaultdict(list) for _ in range(len(slot_df))]
        ...
        if any(self._canonical_sensor_name(name) == 'soc' for name in slot_records):
            soc_name = next(name for name in slot_records if self._canonical_sensor_name(name) == 'soc')
            soc_series = slot_records[soc_name].set_index('timestamp')['cumulative_value'].reindex(slots).ffill()
    ...
    def calculate_metrics(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            last_slot = conn.execute(
                "SELECT MAX(slot_start) FROM slot_observations"
            ).fetchone()
...
class DeterministicSimulator:
    ...
    def __init__(self, learning_engine: LearningEngine):
        ...
        self.capacity_kwh = (
            system_battery.get('capacity_kwh')
            or fallback_battery.get('capacity_kwh')
            or 10.0
        )
        self.min_soc_percent = (
            system_battery.get('min_soc_percent')
            or fallback_battery.get('min_soc_percent')
            or 15.0
        )
        self.cycle_cost = self.config.get('battery_economics', {}).get('battery_cycle_cost_kwh', 0.20)
    ...
    def _fetch_price_rows(self, day: datetime.date) -> List[Tuple[pd.Timestamp, pd.Timestamp, Dict[str, float]]]:
        sql = """
        ...
        with sqlite3.connect(self.engine.db_path) as conn:
            for slot_start, slot_end, import_price, export_price in conn.execute(sql, (day.isoformat(),)):
                ...
                rows.append(
                    (
                        ...
                        end_ts,
                        {
                            'import_price': float(import_price) if import_price is not None else 0.0,
                            'export_price': float(export_price) if export_price is not None else float(import_price or 0.0),
                        },
                    )
                )
        return rows

    def _fetch_forecast_rows(self, day: datetime.date) -> List[Tuple[pd.Timestamp, Dict[str, float]]]:
        sql = """
        ...
        with sqlite3.connect(self.engine.db_path) as conn:
            for slot_start, pv_kwh, load_kwh, temp_c, row_id in conn.execute(sql, (day.isoformat(),)):
    ...
    def _get_stored_historical_data(self, start_date: datetime, end_date: datetime) -> Dict[str, list]:
        with sqlite3.connect(self.engine.db_path) as conn:
            obs_query = """
...
class LearningLoops:
    ...
    def forecast_calibrator(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            current_load_margin = self.config.get('forecasting', {}).get('load_safety_margin_percent', 110.0)
            ...
            if load_change > load_change_cap:
                delta = load_change_cap if new_load_margin > current_load_margin else -load_change_cap
    ...
    def threshold_tuner(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Current values
            current_battery_margin = self.config.get('decision_thresholds', {}).get('battery_use_margin_sek', 0.10)
            current_export_margin = self.config.get('decision_thresholds', {}).get('export_profit_margin_sek', 0.05)
            
            # Search grid around current values
            search_space = []
            for battery_margin in [current_battery_margin - 0.02, current_battery_margin, current_battery_margin + 0.02]:
                for export_margin in [current_export_margin - 0.02, current_export_margin, current_export_margin + 0.02]:
                    # Apply bounds
            ...
            # Check improvement
            improvement = (baseline_objective - best_objective) / baseline_objective if baseline_objective > 0 else 0
            ...
            if improvement >= min_improvement and best_candidate:
                ...
                battery_change = abs(best_candidate['decision_thresholds.battery_use_margin_sek'] - current_battery_margin)
                export_change = abs(best_candidate['decision_thresholds.export_profit_margin_sek'] - current_export_margin)
    ...
    def s_index_tuner(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if improvement >= min_improvement and best_candidate:
                ...
                if (base_change <= base_change_cap and 
                    pv_change <= pv_change_cap and 
                    temp_change <= temp_change_cap):
    ...
    def export_guard_tuner(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Current value
            current_buffer = self.config.get('arbitrage', {}).get('future_price_guard_buffer_sek', 0.00)
            ...
            with sqlite3.connect(self.engine.db_path) as conn:
                ...
                """
                export_events = conn.execute(export_query, (start_date.date().isoformat(), end_date.date().isoformat())).fetchall()
                ...
                for slot_start, export_kwh, export_price, pv_forecast, load_forecast in export_events:
                    ...
                    if future_result and future_result[0] is not None:
                        ...
                        if future_price > float(export_price) + dynamic_threshold:
                            premature_exports += 1
                            missed_profit += float(export_kwh) * max(0.0, future_price - float(export_price))
            ...
            for buffer_delta in [-0.05, 0, 0.05]:
                ...
                search_space.append({
                    'arbitrage.future_price_guard_buffer_sek': new_buffer
                })
            ...
            if improvement >= min_improvement and best_candidate:
                ...
                buffer_change = abs(best_candidate['arbitrage.future_price_guard_buffer_sek'] - current_buffer)
        ...
        try:
            ...
            with sqlite3.connect(self.engine.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE id = ?
                """, (job_end.isoformat(), 'completed', json.dumps({
    ...
    def _apply_changes(self, changes: Dict[str, Any], loop_results: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            for key_path, new_value in changes.items():
                ...
                current_new = config
                for key in keys[:-1]:
                    current_orig = current_orig.get(key, {}) if isinstance(current_orig, dict) else {}
