...
def apply_manual_plan(
    ...
    """
    Apply user-painted manual actions onto the provided DataFrame.

    Args:
        df: Prepared DataFrame indexed by ``start_time``.
        manual_plan: Payload from UI (list or dict) describing manual actions.
        config: Planner configuration (used for timezone + power caps).

    Returns:
        DataFrame with ``manual_action``, ``charge_kw`` and ``water_heating_kw`` annotated.
    """
    if config is None:
        config = {}
    timezone_name = config.get("timezone", "Europe/Stockholm")
    tz = pytz.timezone(timezone_name)
    charge_cap_kw = config.get("system", {}).get("battery", {}).get("max_charge_power_kw", 0.0)
    ...
    working_df["manual_action"] = working_df.get(
        "manual_action", pd.Series([None] * len(working_df), index=working_df.index)
    )
    ...
    for entry in manual_items:
        action = entry.get("content") or entry.get("action")
        if not action:
            continue
        ...
        if normalized_action == "charge":
            working_df.loc[mask, "charge_kw"] = charge_cap_kw
        elif normalized_action == "water heating":
            working_df.loc[mask, "water_heating_kw"] = water_kw_default

        working_df.loc[mask, "manual_action"] = normalized_action
