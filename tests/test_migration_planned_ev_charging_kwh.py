"""Alembic migration c5e8d2a7f913: add planned_ev_charging_kwh to slot_plans."""
...
PREV_REVISION = "bb3329253f22"
REVISION = "c5e8d2a7f913"
COLUMN = "planned_ev_charging_kwh"
...
def _rows(db_path: Path) -> list[tuple]:
    with sqlite3.connect(db_path) as conn:
        return conn.execute(
            "SELECT slot_start, planned_charge_kwh, planned_soc_percent, "
            "planned_water_heating_kwh FROM slot_plans ORDER BY slot_start"
        ).fetchall()
...
def test_upgrade_adds_nullable_column_and_keeps_data(alembic_db: tuple[Config, Path]) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        conn.executemany(
            ...
            "planned_water_heating_kwh) VALUES (?, ?, ?, ?)",
            [
                ("2026-09-24T10:00:00+02:00", 0.5, 60.0, 0.25),
                ("2026-09-24T10:15:00+02:00", 0.0, 61.0, 0.0),
            ],
        )
