...
def learning_engine(temp_db):
    ...
    temp_config = {
        ...
        "decision_thresholds": {"battery_use_margin_sek": 0.10, "export_profit_margin_sek": 0.05},
        "s_index": {
            "base_factor": 1.05,
        ...
        "arbitrage": {"future_price_guard_buffer_sek": 0.00},
    }
...
class TestLearningEngine:
    ...
    def test_etl_cumulative_to_slots(self, learning_engine):
        ...
        assert (
            abs(total_import_delta - 11.2) < 0.1
        ), f"Total import delta should be ~11.2, got {total_import_delta}"
        assert (
            abs(total_pv_delta - 4.8) < 0.1
        ), f"Total PV delta should be ~4.8, got {total_pv_delta}"
    ...
    def test_get_stored_historical_data(self, simulator):
        ...
        with sqlite3.connect(simulator.engine.db_path) as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (now.isoformat(), (now + timedelta(minutes=15)).isoformat(), 1.5, 0.5, 2.0, 3.0),
            )
            conn.commit()
...
class TestErrorHandling:
    ...
    def test_learning_engine_disabled(self):
        ...
        temp_config = {"learning": {"enable": False}, "timezone": "Europe/Stockholm"}
    ...
    def test_database_connection_error(self):
        ...
        # Use invalid path
        temp_config = {
            "learning": {"enable": True, "sqlite_path": "/invalid/path/test.db"},
            "timezone": "Europe/Stockholm",
        }
