...
class TestPowerLimits:
    ...
    def test_pass3_respects_discharge_power_limit(self):
        ...
        rate_limited_output = (
            self.planner.battery_config['max_discharge_power_kw'] * 0.25
        )
        expected_output_kwh = rate_limited_output
        expected_battery_use = self.planner._battery_energy_for_output(expected_output_kwh)
        expected_soc = pytest.approx(self.planner.state['battery_kwh'] - expected_battery_use, rel=1e-5)
    ...
    def test_battery_export_without_pv_surplus(self):
        ...
        expected_soc = pytest.approx(self.planner.state['battery_kwh'] - battery_energy_used, rel=1e-5)
