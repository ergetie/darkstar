...
def test_schema_creation(learning_engine):
    ...
    with sqlite3.connect(learning_engine.db_path) as conn:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='training_episodes'")
        assert cursor.fetchone() is not None
...
def test_store_training_episode(learning_engine):
    ...
    with sqlite3.connect(learning_engine.db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT inputs_json FROM training_episodes WHERE episode_id=?", (episode_id,))
        row = cursor.fetchone()
