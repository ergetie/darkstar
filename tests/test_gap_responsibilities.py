...
class TestGapResponsibilities:
    ...
    def test_price_aware_gap_calculation(self):
        ...
        expected_gap = economic_slots['adjusted_load_kwh'].sum() - economic_slots['adjusted_pv_kwh'].sum()
    ...
    def test_cascading_inheritance(self):
        ...
        self.planner.window_responsibilities = [
            {'total_responsibility_kwh': 2.0, 'window': {'start': pd.Timestamp('2025-01-01 00:00'), 'end': pd.Timestamp('2025-01-01 01:00')}},
            {'total_responsibility_kwh': 3.0, 'window': {'start': pd.Timestamp('2025-01-01 02:00'), 'end': pd.Timestamp('2025-01-01 03:00')}},
            {'total_responsibility_kwh': 1.5, 'window': {'start': pd.Timestamp('2025-01-01 04:00'), 'end': pd.Timestamp('2025-01-01 05:00')}}
        ]
        ...
        for i in range(len(self.planner.window_responsibilities) - 2, -1, -1):
            ...
            if adjusted_next_resp > max_energy:
                self.planner.window_responsibilities[i]['total_responsibility_kwh'] += adjusted_next_resp

        # Verify cascading occurred
        assert self.planner.window_responsibilities[0]['total_responsibility_kwh'] >= 2.0  # Should have increased
