    ...
    for day_offset in range(1, days + 1):
        ...
        for hour in range(24):
            for quarter in range(4):
                ts = tz.localize(
                    datetime(target_date.year, target_date.month, target_date.day, hour, quarter * 15)
                )
...
def test_target_soc_pv_deficit_increases_buffer():
    ...
    factor_deficit, debug_deficit = calculate_target_soc_risk_factor(df_deficit, base_cfg, "Europe/Stockholm")
    
    # Low deficit: PV covers most of load
    df_surplus = build_test_df(load_kwh=30.0, pv_kwh=25.0)
    factor_surplus, debug_surplus = calculate_target_soc_risk_factor(df_surplus, base_cfg, "Europe/Stockholm")
