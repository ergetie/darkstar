...
def test_export_threshold():
    ...
    input_data = KeplerInput(
        slots=slots,
        initial_soc_kwh=5.0 # Full battery
    )
    
    # Case 1: Low Threshold -> Should Export
    config_low = KeplerConfig(
        capacity_kwh=10.0, min_soc_percent=0, max_soc_percent=100,
        max_charge_power_kw=10, max_discharge_power_kw=10,
...
def test_ramping_cost():
    ...
    for i, p in enumerate(prices):
        slots.append(KeplerInputSlot(
            ...
            import_price_sek_kwh=p, export_price_sek_kwh=p
        ))
    ...
    config_no_ramp = KeplerConfig(
        capacity_kwh=10.0, min_soc_percent=0, max_soc_percent=100,
        max_charge_power_kw=4.0, max_discharge_power_kw=4.0, # 1 kWh per slot
        charge_efficiency=1.0, discharge_efficiency=1.0,
        wear_cost_sek_per_kwh=0.0,
    ...
    config_ramp = KeplerConfig(
        capacity_kwh=10.0, min_soc_percent=0, max_soc_percent=100,
        max_charge_power_kw=4.0, max_discharge_power_kw=4.0,
        charge_efficiency=1.0, discharge_efficiency=1.0,
        wear_cost_sek_per_kwh=0.0,
