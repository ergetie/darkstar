...
class PlannerPipeline:
    ...
    def generate_schedule(
        ...
        initial_soc_kwh = float(initial_state.get("battery_kwh", initial_state.get("battery_soc_kwh", 0.0)))
        ...
        if future_df.empty:
            ...
            logger.info("Kepler: Planning %d future slots starting from %s", len(future_df), now_slot)
        ...
        if result.slots:
            logger.info("Kepler result: %d slots, first soc_kwh=%.3f", len(result.slots), result.slots[0].soc_kwh)
        
        # Convert result back to DataFrame
        capacity = kepler_config.capacity_kwh
        result_df = kepler_result_to_dataframe(result, capacity, initial_soc_kwh)
        
        logger.info("result_df first projected_soc_kwh: %.3f", result_df.iloc[0]['projected_soc_kwh'] if len(result_df) > 0 else 0.0)
        
        # Preserve water_heating_kw before merge (Kepler doesn't know about water heating)
        water_heating_series = future_df["water_heating_kw"].copy() if "water_heating_kw" in future_df.columns else None
        ...
        if save_to_file:
            ...
            if mode == "full":
                ...
                s_index_debug["soc_target_percent"] = target_soc_pct if 'target_soc_pct' in dir() else 0.0
