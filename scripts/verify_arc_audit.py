...
def main():
    ...
    print("\n--- ARC3 ---")
    if not check_content("backend/api/routers/services.py", "print\(", "No print() in routers", invert=True): failed += 1
    # We allow 'except Exception' but not bare 'except:'
    # Regex for bare except: except:\s*$
    if not check_content("backend/api/routers/forecast.py", "except:\s*$", "No bare except in forecast.py", invert=True): failed += 1
