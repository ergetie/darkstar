...
def _get_timezone() -> pytz.BaseTzInfo:
    ...
    except: return pytz.timezone("Europe/Stockholm")

def _get_engine_and_config():
    engine = None
    try: engine = get_learning_engine()
    except Exception as exc: 
        logger.warning("Failed to get learning engine: %s", exc)
    try: config = _load_yaml("config.yaml")
    except: config = {}
...
def _compute_risk_profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try: base_factor = float(base_factor_raw)
    except (TypeError, ValueError): base_factor = 1.0

    if risk_appetite <= 1: persona = "Safety First"
    elif risk_appetite == 2: persona = "Conservative"
    elif risk_appetite == 3: persona = "Balanced"
    elif risk_appetite == 4: persona = "Aggressive"
    else: persona = "Gambler"
    ...
    try:
        if os.path.exists("schedule.json"):
            with open("schedule.json", "r") as f:
                ...
                if current_factor is None: current_factor = s_index_meta.get("factor")
                raw_factor = s_index_meta.get("raw_factor")
                if raw_factor is None: raw_factor = s_index_meta.get("factor_unclamped")
    except Exception as exc:
...
def _fetch_weather_volatility(start_time: datetime, end_time: datetime, config: Dict[str, Any]) -> Dict[str, float]:
    try:
...
def _fetch_horizon_series(engine: Any, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if slot_start < now: slot_start += timedelta(minutes=15)
    ...
    for rec in records:
        ...
        slots.append({
            ...
            "probabilistic": {
                ...
                "load_p10": float(rec.get("load_p10")) if rec.get("load_p10") is not None else None,
                "load_p90": float(rec.get("load_p90")) if rec.get("load_p90") is not None else None,
            },
        })
    ...
    try:
        with sqlite3.connect(db_path, timeout=30.0) as conn:
            ...
            for date_str, pv_corr, load_corr in cursor.fetchall():
                ...
                rows.append({"date": date_str, "total_correction_kwh": pv+load, "pv_correction_kwh": pv, "load_correction_kwh": load})
    except Exception as exc:
        logger.warning("Failed to fetch correction history: %s", exc)
    return rows

def _compute_metrics(engine: Any, days_back: int = 7) -> Dict[str, Optional[float]]:
    db_path = getattr(engine, "db_path", None) if engine else None
    metrics = {"mae_pv_aurora": None, "mae_pv_baseline": None, "mae_load_aurora": None, "mae_load_baseline": None}
    if not db_path or not os.path.exists(db_path): return metrics
    ...
    try:
        with sqlite3.connect(db_path, timeout=30.0) as conn:
            ...
            """, (start_time.isoformat(), now.isoformat())).fetchall()
...
def get_aurora_briefing_text(dashboard: Dict[str, Any], config: Dict[str, Any], secrets: Dict[str, Any]) -> str:
    advisor_cfg = config.get("advisor") or {}
    if not advisor_cfg.get("enable_llm", False): return "Aurora briefing is disabled in config."
    
    api_key = secrets.get("openrouter_api_key")
    if not api_key: return "OpenRouter API Key missing."
    
    model = advisor_cfg.get("model", "google/gemini-flash-1.5")
    system_prompt = "You are Aurora, an intelligent energy agent. Summarize status and risk in 2 sentences."
    
    try:
        resp = requests.post(
            "https://openrouter.ai/api/v1/chat/completions",
            headers={"Authorization": f"Bearer {api_key}", "HTTP-Referer": "https://github.com/darkstar"},
            json={"model": model, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": json.dumps(dashboard)}]},
            timeout=15
        )
...
async def aurora_dashboard():
        ...
        if engine is not None and hasattr(engine, "store"):
            ...
            def proc(df):
                ...
                for c in ["p10", "p90", "forecast", "actual"]:
                    if c in f.columns: f[c] = f[c].astype(object).where(pd.notnull(f[c]), None)
    ...
    try:
        ...
        prices = [p for p in get_nordpool_data() if p["start_time"] >= now.replace(hour=0, minute=0, second=0, microsecond=0)]
        if prices:
            spread = max(p["export_price_sek_kwh"] for p in prices) - min(p["import_price_sek_kwh"] for p in prices)
    ...
    try:
        if engine and hasattr(engine, "store"):
            ...
            if len(df) >= 50: metrics["forecast_bias"] = round(df["error"].mean(), 3)
    except: pass
...
async def aurora_briefing(request: Request):
    ...
    try: secrets = _load_yaml("secrets.yaml")
    except: secrets = {}
...
async def forecast_day(date: Optional[str] = None):
    ...
    try:
        ...
        for row in obs_rows:
            slot_start = row[0]
            slots[slot_start] = {"slot_start": slot_start, "actual_pv": row[1], "actual_load": row[2]}
