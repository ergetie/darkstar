    ...
    try:
        ...
        for slot in payload.get("schedule", []):
            ...
            try:
                ...
                if start.tzinfo is None: local = tz.localize(start)
                else: local = start.astimezone(tz)
                if local.date() != today_local: continue
    ...
    try:
        ...
        if os.path.exists(db_path):
             with sqlite3.connect(db_path) as conn:
                conn.row_factory = sqlite3.Row
                # Minimal query
                cur = conn.execute("SELECT slot_start, pv_forecast_kwh, load_forecast_kwh FROM slot_forecasts WHERE slot_start >= ? AND forecast_version = ?", 
                                   (tz.localize(datetime.combine(today_local, datetime.min.time())).isoformat(), active_version))
                for row in cur:
                    try:
