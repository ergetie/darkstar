...
async def clear_quick_action(action: Optional[str] = None):
    executor = _get_executor()
    if not executor: 
        raise HTTPException(500, "Executor unavailable")
...
async def pause_executor(payload: PauseRequest):
    ...
    if hasattr(executor, 'pause'):
        ...
        return {"status": "success", "paused_until": paused_until.isoformat() if paused_until else None}
...
async def get_history(limit: int = 100, offset: int = 0, slot_start: Optional[str] = None, success_only: Optional[str] = None):
    executor = _get_executor()
...
async def get_executor_config():
    ...
    return {
        ...
        "inverter": {
            ...
            "grid_charging_entity": cfg.inverter.grid_charging_entity if hasattr(cfg, 'inverter') else None,
            "max_charging_current_entity": getattr(cfg.inverter, 'max_charging_current_entity', None) if hasattr(cfg, 'inverter') else None,
            "max_discharging_current_entity": getattr(cfg.inverter, 'max_discharging_current_entity', None) if hasattr(cfg, 'inverter') else None,
        } if hasattr(cfg, 'inverter') else {},
        "water_heater": {
            "target_entity": cfg.water_heater.target_entity if hasattr(cfg, 'water_heater') else None,
        ...
        } if hasattr(cfg, 'water_heater') else {},
        "notifications": {
            "service": cfg.notifications.service if hasattr(cfg, 'notifications') else None,
            "on_charge_start": getattr(cfg.notifications, 'on_charge_start', False) if hasattr(cfg, 'notifications') else False,
            "on_charge_stop": getattr(cfg.notifications, 'on_charge_stop', False) if hasattr(cfg, 'notifications') else False,
            "on_export_start": getattr(cfg.notifications, 'on_export_start', False) if hasattr(cfg, 'notifications') else False,
            "on_export_stop": getattr(cfg.notifications, 'on_export_stop', False) if hasattr(cfg, 'notifications') else False,
            "on_error": getattr(cfg.notifications, 'on_error', False) if hasattr(cfg, 'notifications') else False,
        } if hasattr(cfg, 'notifications') else {},
    }
