...
import { Gauge, CalendarRange, BookOpenCheck, Bug, Settings, Bolt, Menu, X, Activity, FlaskConical, Brain } from 'lucide-react'
...
export default function Sidebar(){
    ...
    return (
        ...
        <aside className="hidden lg:block fixed left-6 top-6 bottom-6 z-50">
            ...
            <Item to="/aurora" icon={Brain} label="Aurora" />
        ...
        {mobileOpen && (
            ...
            <nav className="px-4 py-4 space-y-2 text-sm">
                ...
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left ${
                    pathname === '/' ? 'bg-accent text-[#0F1216]' : 'bg-surface border border-line/60 text-muted'
                }`}
                onClick={closeMobile}
                >
                ...
                type="button"
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left ${
                    pathname === '/planning' ? 'bg-accent text-[#0F1216]' : 'bg-surface border border-line/60 text-muted'
                }`}
                onClick={closeMobile}
                >
                <span className="flex items-center gap-2">
                ...
                type="button"
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left ${
                    pathname === '/aurora' ? 'bg-accent text-[#0F1216]' : 'bg-surface border border-line/60 text-muted'
                }`}
                onClick={closeMobile}
                >
                <span className="flex items-center gap-2">
                ...
                type="button"
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left ${
                    pathname === '/learning' ? 'bg-accent text-[#0F1216]' : 'bg-surface border border-line/60 text-muted'
                }`}
                onClick={closeMobile}
                >
                <span className="flex items-center gap-2">
                ...
                type="button"
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left ${
                    pathname === '/debug' ? 'bg-accent text-[#0F1216]' : 'bg-surface border border-line/60 text-muted'
                }`}
                onClick={closeMobile}
                >
                <span className="flex items-center gap-2">
                ...
                type="button"
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left ${
                    pathname === '/settings' ? 'bg-accent text-[#0F1216]' : 'bg-surface border border-line/60 text-muted'
                }`}
                onClick={closeMobile}
                >
                <span className="flex items-center gap-2">
